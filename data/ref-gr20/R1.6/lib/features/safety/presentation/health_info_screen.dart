...
/// Les donnees sont stockees localement (Hive) et ne quittent
  ...
  Future<void> _save() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Informations sauvegardees'),
          backgroundColor: AppTheme.vertMaquis,
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(color: AppTheme.vertMaquisLight.withAlpha(80)),
                      ),
                      child: const Row(
                        children: [
                          Icon(Icons.lock, color: AppTheme.vertMaquisLight, size: 20),
                          SizedBox(width: AppTheme.spacingSm),
                          Expanded(
                            child: Text(
                              'Ces donnees restent sur votre telephone. '
                              'Elles ne sont jamais envoyees sur internet.',
                              style: TextStyle(fontSize: 13, color: AppTheme.vertMaquisLight),
                            ),
                          ),
                        ],
                      ),
  ...
  Widget _buildField({required TextEditingController controller, required String label, required String hint, required IconData icon, int maxLines = 1}) {
    return TextFormField(
      ...
      decoration: InputDecoration(
        ...
        prefixIcon: Icon(icon, color: AppTheme.vertMaquisLight),
        ...
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(AppTheme.radiusInput), borderSide: const BorderSide(color: AppTheme.vertMaquisLight, width: 2)),
      ),
    );
  }
}
