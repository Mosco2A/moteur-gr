...
class MemberPositionCard extends StatelessWidget {
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  member.displayName ?? 'Randonneur anonyme',
                  style: theme.textTheme.titleSmall,
                ),
                const SizedBox(height: AppTheme.spacingXs),
                Text(
                  _formatCoordinates(member.lastLat, member.lastLng),
                  style: theme.textTheme.bodySmall?.copyWith(
                    color: AppTheme.grisTexteSecondaire,
                  ),
                ),
                if (member.currentStageId != null)
                  Padding(
                    padding: const EdgeInsets.only(top: AppTheme.spacingXs),
                    child: Text(
                      'Etape: ${member.currentStageId}',
  ...
  _Freshness _computeFreshness(String lastUpdate) {
    try {
      ...
      return const _Freshness(color: AppTheme.grisGranite, label: '?');
    }
  }
}
