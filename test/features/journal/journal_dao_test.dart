  ...
  group('JournalDao', () {
    test('insertEntry insère une entrée correctement', () async {
      final id = await dao.insertEntry(
        makeEntry(trailId: 'gr20', stageNumber: 1),
      );
      expect(id, greaterThan(0));

      final entries = await dao.getByTrailId('gr20');
    ...
    test('getByTrailId filtre par sentier', () async {
      await dao.insertEntry(makeEntry(trailId: 'gr20', stageNumber: 1));
      await dao.insertEntry(makeEntry(trailId: 'tmb', stageNumber: 1));
      await dao.insertEntry(makeEntry(trailId: 'gr20', stageNumber: 2));
    ...
    test('getByStage filtre par étape', () async {
      ...
      final stage1 = await dao.getByStage('gr20', 1);
      final stage2 = await dao.getByStage('gr20', 2);
    ...
    test('updateEntry met à jour le contenu', () async {
      final id = await dao.insertEntry(
        makeEntry(trailId: 'gr20', stageNumber: 1, content: 'avant'),
      );
    ...
    test('deleteEntry supprime l\'entrée', () async {
      ...
      await dao.insertEntry(
        makeEntry(trailId: 'gr20', stageNumber: 2),
      );
    ...
    test('countPhotosToday compte les photos du jour', () async {
      ...
      final count = await dao.countPhotosToday('gr20');
      expect(count, 2);
    });
    ...
    test('canAddPhoto retourne true si < 3 photos', () async {
      ...
      final canAdd = await dao.canAddPhoto('gr20');
      expect(canAdd, true);
    });
    ...
    test('countByTrailId retourne le bon nombre', () async {
      ...
      final count = await dao.countByTrailId('gr20');
      expect(count, 2);
    });
